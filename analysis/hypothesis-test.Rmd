...
```{r}
#| fig.alt = "Latitude hypothesis scheme.",
#| out.width = "25%",
#| echo = FALSE

knitr::include_graphics("_static/hypothesis-scheme.png")
```
...
def print_test_result(test):
    ...
    if test_p_value < 0.05:
        h_result = "Reject H0 in favour of Ha"
    else:
        h_result = "Do not reject H0"
    
    print(
        "> Directional Student's t-test (", test_type, ")", "\n",
        "> ", "\n",
        "> * (H0) Mean Tau North >= Mean Tau South", "\n",
        "> * (Ha) Mean Tau North < Mean Tau South", "\n",
        "> ", "\n",
        "> **", h_result, "**", " ",
        "(p-value = ", "{:1.5f}".format(test_p_value), ")",
        sep = ""
    )
```
